...
def register_youtube_routes(app):
    ...
    try:
        ...
        logger.info(f"[SETTLEMENT] 요청 파라미터 - start: {start_date}, end: {end_date}, editor_id: {editor_id}, status: {settlement_status}")
        ...
        if start_date:
            try:
                ...
                logger.info(f"[SETTLEMENT] start_date 필터 적용: {start_date_obj}")
            except ValueError:
                logger.warning(f"[SETTLEMENT] 유효하지 않은 start_date: {start_date}")

        if end_date:
            try:
                end_date_obj = datetime.strptime(end_date, '%Y-%m-%d').date()
                query = query.filter(Work.work_date <= end_date_obj)
                logger.info(f"[SETTLEMENT] end_date 필터 적용: {end_date_obj}")
            except ValueError:
                logger.warning(f"[SETTLEMENT] 유효하지 않은 end_date: {end_date}")

        # 편집자 필터
        if editor_id:
            query = query.filter(Work.editor_id == editor_id)
            logger.info(f"[SETTLEMENT] editor_id 필터 적용: {editor_id}")

        # 정산 상태 필터
        if settlement_status != 'all':
            query = query.filter(Work.settlement_status == settlement_status)
            logger.info(f"[SETTLEMENT] settlement_status 필터 적용: {settlement_status}")

        # 데이터 조회
        works = query.order_by(Work.work_date.desc()).all()
        logger.info(f"[SETTLEMENT] 조회된 작업 수: {len(works)}")
        ...
        logger.info("[SETTLEMENT] 최종 응답 데이터:\n%s", json.dumps(response_data, ensure_ascii=False, indent=2))

        return jsonify(response_data)

    except Exception as e:
        logger.exception("[SETTLEMENT] 예외 발생:")
        return jsonify({"status": "error", "message": str(e)})
